...
router.post('/join', auth, async (req, res) => {
  try {
    ...
    if (league.members.includes(req.user.id)) return res.status(400).json({ msg: 'Already joined' });
    if (league.members.length >= league.teamCount) return res.status(400).json({ msg: 'League is full' });
